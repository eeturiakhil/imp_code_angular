(function() {
    'use strict';
      ...
      function DashCtrl($scope, $window, $state, $cordovaFile, firstTime, $rootScope, LoginService, SecurityCodeService, $ionicSlideBoxDelegate, userService, $ionicLoading, ionicDatePicker, dashboardService, $ionicPopup){
        var vmDash = this;
        // alert('hi');
        vmDash.dashboard = {};
        ...
        if($window.innerWidth <= 480){
          vmDash.dashboard.graphHeight = 250;
        }
        else{
          vmDash.dashboard.graphHeight = 400;          
        }

        function dashboardGraphData(){
          if($rootScope.online){
            vmDash.dashboard.graphData = [];
         
            $ionicLoading.show({template: '<ion-spinner icon="ios"></ion-spinner>'});
            dashboardService.getGraphData().then(function(data, status, headers, config){
             ...
             if(data.status){
                ...
                vmDash.dashboard.graphData.Bar_Graph = response.bar_graph;
                vmDash.dashboard.graphData.Line_Graph = response.line_graph;

                 vmDash.dashboard.lineData = [];
                 vmDash.dashboard.barData = [];
                 vmDash.dashboard.lineObject = {};
                 vmDash.dashboard.barObject = {};
                 angular.forEach(vmDash.dashboard.graphData.Line_Graph.data, function(value, key){
                    var c = [];
                    var k1 = {};
                    k1.v = key; 
                    var k2 = {};
                    ...
                    var k3 = {};
                    // k3.v = Math.random();
                    k3.v = vmDash.dashboard.graphData.Line_Graph.data1[key];
                    c.push(k1, k2, k3);
                    var c1 = {c};
                    vmDash.dashboard.lineData.push(c1);
                 });

                 console.log(vmDash.dashboard.lineData);

                 var color = $rootScope.colors.length-1;
                 angular.forEach(vmDash.dashboard.graphData.Bar_Graph.data, function(value, key){
                    // for (var i = $rootScope.colors.length - 1; i >= 0; i--) {
                    var c = [];
                    var k1 = {};
                    k1.v = key; 
                    var k2 = {};
                 ...
                 vmDash.dashboard.lineObject.data = {
                    ...
                    id: "today",
                    label: vmDash.dashboard.graphData.Line_Graph.title1,
                    type: "number"
                    },
                    {
                    id: "LWD",
                    label: vmDash.dashboard.graphData.Line_Graph.title2,
                    type: "number"
                    }],
                 ...
                 vmDash.dashboard.lineObject.options = {
                      'curveType': 'function',
                      'pointSize': 5,                  
                      'title': vmDash.dashboard.graphData.Line_Graph.title,
                      'chartArea':{left:0,top:20, bottom:50, width:'100%',height:'100%'},
                      'colors':['#1ccdb9','#f68884'],
                      'height': vmDash.dashboard.graphHeight,
                      // 'tooltip' : {
                      //    trigger: 'none'
                      // },
                      'tooltip': {
                      ...
                      color: $rootScope.graph_color,
                      fontSize: 16,
                      fontName: '-apple-system, Helvetica Neue, Roboto, Segoe UI, sans-serif'
                      },                  
                 ...
                 vmDash.dashboard.barObject.options = {
                      'title': vmDash.dashboard.graphData.Bar_Graph.title,
